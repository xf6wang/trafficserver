...
parse_generic_response(OpType optype, int fd)
{
  ...
  err = recv_mgmt_message(MgmtMessageType::RESPONSE, data.ptr, data.len, optype, &ival);
  ats_free(data.ptr);
...
event_poll_thread_main(void *arg)
{
  ...
  while (true) {
    ...
    ret = recv_mgmt_message(MgmtMessageType::REQUEST, reply.ptr, reply.len, OpType::EVENT_NOTIFY, &optype, &name, &desc);
    ats_free(reply.ptr);
